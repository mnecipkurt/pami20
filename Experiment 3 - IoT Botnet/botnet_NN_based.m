...
h = (-1.5:0.02:6.4)';
no_trials = 80; % number of trials
sum_fap = zeros(length(h),1);
for nn = 1:no_trials
    nn
    sliding_data_window = X(randi(N,K,1),:);  % Initial Nominal Data Window
    alarm_flag = zeros(length(h),1);
    t = 1;
    while (alarm_flag(length(h)) == 0)  
        sliding_data_window(1:K-1,:) = sliding_data_window(2:K,:);
        ind_rand = randi(N,1);
        nominal_point = X(ind_rand,:); % nominal data point
        sliding_data_window(K,:) = nominal_point; % nominal data point
...
for nn = 1:num_trials
    ...
    while (alarm_flag(length(h)) == 0 || t <= max_tolerable_delay4)     
        ...
        anomalous_point = X_anomalous(ind_rand,:);
        sliding_data_window(K,:) = anomalous_point;
        NN_mtrx = zeros(K,K);   % k nearest neighbor matrix: NNs take 1, others take 0 for each data point
        for i = 1:K
            for j = 1:K
                NN_mtrx(i,j) = norm((sliding_data_window(i,:)-sliding_data_window(j,:)),2);
...
% sliding_data_window = X(randi(N,K,1),:);
% while (t <= tmax)
%     t
%     if (t < tau) 
%         sliding_data_window(1:K-1,:) = sliding_data_window(2:K,:);
